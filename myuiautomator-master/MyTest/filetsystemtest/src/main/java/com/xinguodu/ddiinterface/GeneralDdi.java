...
public final class GeneralDdi {  
    ...
    public int ddi_sys_write_file(byte[] name,int name_len,byte[] file,int file_len){
		int nCmd;
		int ret;
        int wlen = 0;
		byte[] wbuf = new byte[4096];
        ...
        intTmp = b_htonl(name_len);
		System.arraycopy(intTmp, 0, wbuf,wlen, 4);
        wlen += 4;

        System.arraycopy(name, 0, wbuf,wlen, name_len);
        wlen += name_len;

        intTmp = b_htonl(file_len);
		System.arraycopy(intTmp, 0, wbuf,wlen, 4);
        wlen += 4;

        System.arraycopy(file, 0, wbuf,wlen, file_len);
        wlen += file_len;
    ...
    public int ddi_sys_read_file(byte[] name,int name_len,byte[] file,int[] file_len){
		int nCmd;
		int ret;
        int wlen = 0;
		byte[] wbuf = new byte[4096];
		...
		ret = ddi.ddi_general_interface(nCmd,wlen,wbuf,file_len,file,SpRet);
		if(ret != 0){
			Log.e(TAG, Integer.toHexString(nCmd)+" cmd err,ret = "+ret+" spRet = "+SpRet[0]);
			return ret;
		}
